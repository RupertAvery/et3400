...
void BreakpointsDialog::setupUi()
{
    ...
    connect(add_action, &QAction::triggered, this, &BreakpointsDialog::add_breakpoint_from_table);
    connect(tab_remove_breakpoint_action, &QAction::triggered, this, &BreakpointsDialog::remove_breakpoint_from_table);
    ...
    connect(breakpoints_table, &QTableWidget::itemSelectionChanged, this, &BreakpointsDialog::breakpoints_table_selection_changed);
...
void BreakpointsDialog::remove_breakpoint_from_table()
{
	int row = breakpoints_table->currentRow();
	if (row < 0)
		return;
...
void BreakpointsDialog::breakpoints_table_selection_changed()
{
...
void BreakpointsDialog::add_breakpoint_from_table()
{
	bool ok;
	...
	debugger->emu_ptr->add_breakpoint(address);
	populate_breakpoints_table();
}
